        ...
        <div class="main">
          ...
          <div class="extra">
            <el-button
              ...
              >已关注</el-button
            >
            <el-button type="primary" class="button" round size="large" v-else @click="follow(item.uid, index, -1)"
              >关注</el-button
            >
          </div>
        </div>
...
const follow = (fid: string, index: number, type: number) => {
  followById(fid).then(() => {
    // type: -1 表示点击"关注"按钮，关注成功后变为已关注状态 (isFollow = true)
    // type: 1 表示点击"已关注"按钮，取消关注后变为未关注状态 (isFollow = false)
