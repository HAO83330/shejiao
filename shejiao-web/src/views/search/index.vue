      ...
      <ul class="agree-container" v-infinite-scroll="loadMoreUser" :infinite-scroll-distance="20">
        <li class="agree-item" v-for="(item, index) in userDataList" :key="index">
          ...
          <div class="main">
            ...
            <div class="extra">
              <el-button type="primary" round size="large" v-if="item.isFollow" @click="follow(item.id, index, 1)"
                >已关注</el-button
              >
              <el-button type="primary" round size="large" v-else @click="follow(item.id, index, 0)">关注</el-button>
            </div>
          </div>
        </li>
      </ul>
...
import { search as recordSearch } from "@/api/visit";
...
const follow = (fid: string, index: number, type: number) => {
  followById(fid).then(() => {
    userDataList.value[index].isFollow = type == 0;
  });
};
...
const getNoteListByKeyword = () => {
  ...
  if (noteDTO.value.keyword && userStore.isLogin()) {
    ...
    recordSearch(userInfo.id, noteDTO.value.keyword).catch((err) => {
