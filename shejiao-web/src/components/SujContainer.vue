        ...
        <div class="history-list">
          <div v-for="(item, index) in historyRecordList" :key="index">
            <div class="history-item">
              ...
              <!---->
              <span class="close-tag" v-show="showTagState" @click.stop="deleteRecord(index)">X</span>
            </div>
          </div>
          <!---->
        </div>
...
import { getHotRecord, addRecord, deleteRecord as deleteRecordApi, clearAllRecord, getRecordByKeyWord, getGuessYouWant } from "@/api/search";
...
const deleteRecord = (index: number) => {
  ...
  const keyword = historyRecordList.value[index];
  console.log('删除搜索记录:', { uid, keyword, index });
  if (!uid) {
    console.warn('用户未登录，无法删除搜索记录');
    return;
  }
  deleteRecordApi(keyword, uid).then(() => {
...
watch(
  ...
  () => {
    ...
    const uid = userStore.getUserInfo()?.id;
    if (uid) {
      getRecordByKeyWord("", uid).then((res) => {
        historyRecordList.value = res.data.map((item: any) => item.content);
      });
      // 获取个性化推荐
      getGuessYouWant(uid).then((res) => {
        recommendRecords.value = res.data.map((item: any) => item.content);
      }).catch(error => {
        console.error('获取个性化推荐失败:', error);
        // 降级处理，使用默认推荐
        recommendRecords.value = ["壁纸", "风景", "情侣", "头像", "动漫", "动物", "美食", "旅行"];
      });
    } else {
      // 新用户或无用户信息，使用默认推荐
      recommendRecords.value = ["壁纸", "风景", "情侣", "头像", "动漫", "动物", "美食", "旅行"];
...
const searchPage = (keyword: string) => {
  const uid = userStore.getUserInfo()?.id;
  addRecord(keyword, uid);
  searchStore.setKeyword(keyword);
  searchStore.pushRecord(keyword, uid);
