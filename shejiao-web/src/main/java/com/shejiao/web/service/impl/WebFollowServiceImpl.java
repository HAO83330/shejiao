...
import com.shejiao.web.domain.vo.FollowVO;
import com.shejiao.web.domain.vo.TrendVO;
...
import com.shejiao.web.service.IWebFollowService;
...
public class WebFollowServiceImpl extends ServiceImpl<WebFollowMapper, WebFollow> implements IWebFollowService {
    ...
    public Page<FollowVO> getFriend(long currentPage, long pageSize, String uid, Integer type) {
        ...
        List<WebFollow> followers = this.list(new QueryWrapper<WebFollow>().eq("uid", currentUserId));
        ...
        followerList.forEach(item -> {
            ...
            if (type == 0) {
                ...
                followVo.setIsFollow(followerSet.contains(item.getUid()));
            } else if (type == 1) {
                user = userMap.get(item.getFid());
                followVo.setIsFollow(followerSet.contains(item.getFid()));
            }
