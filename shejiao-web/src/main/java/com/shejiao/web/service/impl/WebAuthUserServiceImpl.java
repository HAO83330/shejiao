...
import com.shejiao.web.domain.dto.AuthUserDTO;
import com.shejiao.web.domain.entity.WebUser;
import com.shejiao.web.manager.AsyncManager;
import com.shejiao.web.manager.factory.AsyncFactory;
...
public class WebAuthUserServiceImpl extends ServiceImpl<WebUserMapper, WebUser> implements IWebAuthUserService {
    ...
    public Map<String, Object> login(AuthUserDTO authUserDTO) {
        ...
        if (ObjectUtil.isEmpty(authUser)) {
            ...
            if (!authUser.getPassword().equals(SecureUtil.md5(authUserDTO.getPassword()))) {
                throw new shejiaoException(ResultCodeEnum.ERROR_PASSWORD);
            }
            // 校验用户状态
            if ("1".equals(authUser.getStatus())) {
                throw new shejiaoException("用户已被停用，无法登录");
            }
        }
        // 更新登录信息
    ...
    public Map<String, Object> loginByCode(AuthUserDTO authUserDTO) {
        ...
        if (this.checkCode(authUserDTO) || currentUser == null) {
            throw new shejiaoException(AuthConstant.LOGIN_FAIL);
        }
        // 校验用户状态
    ...
    public WebUser register(AuthUserDTO authUserDTO) {
        ...
        // 检查用户名是否存在
        while (this.count(new QueryWrapper<WebUser>().eq("username", username)) > 0) {
